...
import com.example.lovable_clone.config.KubernetesConfig;
import com.example.lovable_clone.dto.deploy.DeployResponse;
import com.example.lovable_clone.service.DeployementService;
...
@Slf4j
public class KubernetesDeployement implements DeployementService {
    ...
    public DeployResponse deploy(Long projectId) {
        ...
        if(existingPod!=null)
        {
            return new DeployResponse("http://"+domain+":8090");//Reverse proxy
        }
    ...
    private DeployResponse claimAndStartPod(Long projectId, String domain) {
        ...
        Pod pod=kubernetesClient.pods().inNamespace(NAMESPACE).withLabel(POOL_LABEL,IDLE).list().getItems().stream().findFirst().orElseThrow(()->new RuntimeException("No idle runners available. Please scale up the runner-pool"));
        String podName=pod.getMetadata().getName();
        log.info("Claiming pod {} for project {}",podName,projectId);

        kubernetesClient.pods().inNamespace(NAMESPACE).withName(podName).editStatus(p->{
    ...
    private Pod findActivePod(Long projectId) {
        return kubernetesClient.pods().inNamespace(NAMESPACE).withLabel(PROJECT_LABEL,projectId.toString()).withLabel(POOL_LABEL,BUSY).list().getItems().stream().filter(pod -> pod.getStatus().getPhase().equals("Running")).findFirst().orElse(null);
    }
